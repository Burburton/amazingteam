...
├── .ai-team/              # Foundation files (copied)
...
├── .github/
│   └── workflows/
│       └── ai-team.yml       # Reference to foundation version
├── ai-team.config.yaml       # User configuration (optional)
└── src/                      # User's actual code

Foundation (loaded at runtime):
├── .ai-team/                 # Downloaded during CI
...
### 1. GitHub Action: `ai-team-action`
...
**Location**: Separate repository `ai-team-action` or included in foundation

**Action Interface**:
```yaml
- uses: your-org/ai-team-action@v2.2.0
  with:
    version: '2.2.0'           # Optional, defaults to action version
    config: 'ai-team.config.yaml'  # Optional, default path
    overlay: 'python-backend'  # Optional, technology overlay
```

### 2. User Configuration: `ai-team.config.yaml`
...
### 3. CLI Tool: `ai-team` (NPM Package)

Command-line tool for local development and upgrade management:

```bash
# Commands
ai-team init                    # Initialize ai-team.config.yaml
ai-team version                 # Show current foundation version
ai-team check-update            # Check for updates
ai-team upgrade [--to x.x.x]    # Upgrade to new version
ai-team validate                # Validate configuration
ai-team local                   # Download foundation for local dev
```
...
│   ├── ai-team.yml            # Template for workflow
│   └── gitignore              # Template for .gitignore additions
├── schemas/                    # NEW: JSON Schemas
│   └── config.schema.json     # Schema for ai-team.config.yaml
├── overlays/
├── docs/
├── AGENTS.md
├── VERSION
...
├── .github/
│   └── workflows/
│       └── ai-team.yml        # Created by CLI
├── opencode.jsonc             # Generated by CLI (for local dev)
├── ai-team.config.yaml        # Created by CLI
├── .ai-team/
...
├── .gitignore                 # Should ignore .ai-team-local/
└── src/
    └── ...                    # User's code
```
...
- `.ai-team/memory/` - Runtime state that changes during execution
...
# Downloads to .ai-team-local/ (gitignored)
...
# 
# Modified files:
# - .github/workflows/ai-team.yml (version reference)
# 
...
| Step | Task | Files | Status |
|------|------|-------|--------|
| 2.1 | Create CLI entry point | `cli/ai-team.cjs` | Pending |
...
| 3.3 | Create workflow template for users | `templates/ai-team.yml` | Pending |
...
async function generateOpenCodeConfig(foundationDir, userConfig, projectDir) {
  ...
  const template = await loadJSON(path.join(foundationDir, '.ai-team/opencode.template.jsonc'));
...
Check NPM registry or GitHub releases
    ...
    ▼
Download tarball:
  https://registry.npmjs.org/ai-team-foundation/-/ai-team-foundation-2.2.0.tgz
  OR
  https://github.com/org/ai-team-foundation/archive/v2.2.0.tar.gz
    │
    ▼
...
{
  "instructions": [".ai-team-local/AGENTS.md"],
  "skills": {
    "test-first-feature-dev": {
      "path": ".ai-team-local/.opencode/skills/test-first-feature-dev/SKILL.md"
    }
  }
}
```
...
# 3. Update .github/workflows/ai-team.yml
# 4. Add .ai-team/ to .gitignore
...
| OpenCode can't find remote files | Local dev broken | Medium | `ai-team local` command downloads for local use |
| NPM registry downtime | CI fails | Low | Fallback to GitHub releases |
| User deletes `opencode.jsonc` | Local dev breaks | Medium | `ai-team validate` catches this |
...
1. User project has < 5 AI-team related files committed
...
- B: Include in ai-team.config.yaml
